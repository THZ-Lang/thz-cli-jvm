...
$Raiz = if (Test-Path "$PSScriptRoot\..\..\thz-lang") { (Resolve-Path "$PSScriptRoot\..\..\thz-lang").Path } else { (Resolve-Path "$PSScriptRoot\..\..\..").Path }
...
if (-not $JavaHome) {
    if (Test-Path "C:\Users\lucas\scoop\apps\graalvm25-jdk\current\bin\jpackage.exe") {
        $JavaHome = "C:\Users\lucas\scoop\apps\graalvm25-jdk\current"
